...
function registerUser(paramUserObject) {
    ...
    */
    return new Promise((resolve, reject) => {
        ...
        knex(process.env.MYSQL_DB_USER_TABLE1).insert(paramUserObject).then(() => {
...
function updateUser(userId) {
    ...
    return new Promise((resolve, reject) => {
        knex(process.env.MYSQL_DB_USER_TABLE1)
...
function setUserOnlineOrOfflineStatus( paramUserUUID , paramOnline, paramTimestamp ){

    return new Promise((resolve, reject) => {
        knex(process.env.MYSQL_DB_USER_TABLE1)
        .update({ user_is_online : paramOnline, last_online_date : paramTimestamp })
        .where({ user_uuid : paramUserUUID})
        .then(function (database_result) {
            resolve(database_result);
        }).catch((e) => {
...
function writeMessage(paramMessageObject) {

    return new Promise((resolve, reject) => {

        knex(process.env.MYSQL_DB_MESSAGE_TABLE1).insert(paramMessageObject).then(() => {
...
function readAllMessages(){
    ...
    return new Promise((resolve, reject) => {
        ...
        knex(process.env.MYSQL_DB_MESSAGE_TABLE1).then(function (database_result) {
            console.log("+++++++++++++++++++++++++++++");
            console.log(database_result)
            console.log("+++++++++++++++++++++++++++++");
            resolve(database_result);
...
function getAllUsersPublicInfoOnly() {
...
module.exports = {
    ...
    registerUser,
    getAllUsers,
    getSelectedUserByUUID,
    setUserOnlineOrOfflineStatus,
    writeMessage,
    readAllMessages,
    getAllUsersPublicInfoOnly
}
